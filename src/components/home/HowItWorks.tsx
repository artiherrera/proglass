...
export function HowItWorks({
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      {videoSrc ? (
        // Con video vertical (TikTok): video + pasos al lado.
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <VerticalVideo src={videoSrc} poster={poster} />
          <ol className="flex flex-col gap-6">
            {STEPS.map((s) => (
              <li key={s.n} className="flex gap-4">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-accent font-display text-lg font-extrabold text-paper">
                  {s.n}
                </span>
                <div>
        ...
        // Sin video: los 3 pasos centrados.
